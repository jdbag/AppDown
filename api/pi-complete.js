export default async function handler(req, res) {
  ...
  if (!paymentId || !txid) {
    return res.status(400).setHeader('Access-Control-Allow-Origin','*')
      .json({error:'paymentId and txid are required', receivedKeys:Object.keys(payload||{}), received:payload});
  }
  const url = `https://api.minepi.com/v2/payments/${encodeURIComponent(paymentId)}/complete`;
  const body = new URLSearchParams({txid});
  const resp = await fetch(url, {method:'POST', headers:{Authorization:`Key ${apiKey}`, 'Content-Type':'application/x-www-form-urlencoded'}, body});
